...
/* harmony import */ var _equal__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(4470);
...
const cache = (len = 100) => {
  ...
  const record = (data) => {
    ...
    const tempData = (0,_clone__WEBPACK_IMPORTED_MODULE_1__/* ["default"] */ .A)(data);
    if (!(0,_equal__WEBPACK_IMPORTED_MODULE_0__/* ["default"] */ .A)(tempData, snapshot[length - 1]?.data)) {
  ...
  const cursor = (num = 0) => {
    ...
    return {
      index: currentIndex,
      length,
      data: (0,_clone__WEBPACK_IMPORTED_MODULE_1__/* ["default"] */ .A)(snapshot[currentIndex]?.data)
    };
  };
  const jump = (index) => {
    currentIndex = index;
  ...
  const undo = () => cursor(-1);
  const redo = () => cursor(1);
  const getList = () => (0,_clone__WEBPACK_IMPORTED_MODULE_1__/* ["default"] */ .A)(snapshot);
  const clean = () => {
    snapshot.length = 0;
    currentIndex = -1;
  };
